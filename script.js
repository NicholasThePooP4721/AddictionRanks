...
function scoreOf(entry) {
  ...
  return voteValues.reduce((sum, val) => sum + Number(val || 0), 0);
}
...
function renderCategorySection(entries, category) {
  ...
  if (categoryEntries.length === 0) {
    ...
    categoryEntries.forEach((entry) => {
      ...
      const session = getSession();
      const alreadyVoted = Boolean(entry.votes?.[session?.code]);

      node.querySelector(".entry-photo").src = entry.photo;
      node.querySelector(".entry-name").textContent = `${entry.username} (${entry.category})`;
      node.querySelector(".entry-profile").textContent = `Profil: ${entry.ownerName}${entry.ownerAbout ? ` - ${entry.ownerAbout}` : ""}`;
  ...
  if (userRows.length === 0) {
    ...
    userRows.forEach((row, index) => {
      ...
      p.textContent = `${index + 1}. ${row.name} (${row.about || "kein Profiltext"}) - Score ${row.score}, Posts ${row.posts}`;
...
function render(entries) {
  ...
  CATEGORIES.forEach((category) => {
    leaderboardsRoot.appendChild(renderCategorySection(entries, category));
  });
}
